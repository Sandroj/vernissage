...
async function fetchWikidataLabels(qids) {
  const entries = await mapLimit(qids, 2, async (batch) => {
    ...
    const response = await fetch(url, { headers: { 'User-Agent': 'VernissageArtCatalogue/1.0 (title metadata research)' } })
...
async function main() {
  ...
  const pageTitles = await mapLimit(sourcedPages, 4, async (row) => {
    const response = await fetch(String(row.source_url), { headers: { 'User-Agent': 'VernissageArtCatalogue/1.0 (title metadata research)' } })
