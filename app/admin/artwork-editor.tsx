    ...
    {artwork && <>
      <form onSubmit={saveArtwork} className="space-y-6 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="rounded-xl border border-dashed border-stone-300 bg-stone-50 p-4 text-sm text-stone-600">
          ...
          <span className="mt-1 block text-xs text-stone-400">Plak een link naar de afbeelding zelf, of naar de pagina waarop hij staat — Seen haalt de afbeelding op en probeert titel, materiaal en afmetingen mee te lezen. Lege velden vullen zich; ingevulde velden blijven staan.</span>
        ...
        <label className="block rounded-xl border border-dashed border-stone-300 bg-stone-50 p-4 text-sm text-stone-600"><span className="flex items-center gap-2 font-semibold"><Upload size={15} /> Nieuwe afbeelding uploaden</span><span className="mt-1 block text-xs text-stone-400">JPEG, PNG of WebP tot 25 MB. Seen draait de afbeelding automatisch recht, schaalt tot 2400 px en comprimeert naar WebP.</span><input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(e) => uploadImage(e.target.files?.[0])} className="mt-3 block w-full text-sm" /></label>
