...
export async function POST(req: Request) {
  ...
  if (!(file instanceof File) || !extensions[file.type] || file.size > 25 * 1024 * 1024) return NextResponse.json({ error: 'Kies een JPEG-, PNG- of WebP-bestand van maximaal 25 MB.' }, { status: 400 })
  ...
  try {
    ...
    return NextResponse.json({ imageUrl: `${R2_PUBLIC_URL.replace(/\/$/, '')}/${key}`, originalBytes: bytes.byteLength, optimizedBytes: optimized.byteLength })
  } catch {
